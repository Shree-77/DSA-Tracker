...
def test_mark_day_done_sets_completed_at(db_session, plan_id):
...
def test_mark_in_progress_clears_completed_at(db_session, plan_id):
    service = StudyDayService(db_session)
...
def test_update_notes(db_session, plan_id):
    service = StudyDayService(db_session)
...
def test_upsert_tracker_creates_and_updates(db_session, plan_id):
...
def test_missing_plan_raises(db_session):
    with pytest.raises(NotFoundError):
        StudyDayService(db_session).get_day(9999, 1)


def test_missing_day_raises(db_session, plan_id):
    with pytest.raises(NotFoundError):
        StudyDayService(db_session).get_day(plan_id, 999)
