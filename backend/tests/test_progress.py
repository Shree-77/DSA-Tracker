...
def test_completion_percentage(db_session, plan_id, test_user_id):
    days_service = StudyDayService(db_session, test_user_id)
    ...
    progress = ProgressService(db_session, test_user_id).get_progress(plan_id, today=START + timedelta(days=6))
...
def test_current_and_longest_streak(db_session, plan_id, test_user_id):
    ...
    progress = ProgressService(db_session, test_user_id).get_progress(plan_id, today=today)
    assert progress.current_streak == 5
    assert progress.longest_streak == 5


def test_incomplete_today_does_not_break_streak(db_session, plan_id, test_user_id):
    service = StudyDayService(db_session, test_user_id)
...
def test_broken_streak(db_session, plan_id, test_user_id):
    service = StudyDayService(db_session, test_user_id)
...
def test_statistics_aggregation(db_session, plan_id, test_user_id):
    ...
    progress = ProgressService(db_session, test_user_id).get_progress(plan_id, today=START)
...
def test_weekly_breakdown(db_session, plan_id, test_user_id):
    service = StudyDayService(db_session, test_user_id)
